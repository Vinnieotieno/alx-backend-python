...
class TestGithubOrgClient(unittest.TestCase):
    ...
    def test_org(self, org_name, mock_get_json):
        """Test GithubOrgClient.org returns expected org payload."""
    ...
    def test_public_repos(self, mock_get_json):
        """Test public_repos returns list of repo names, optionally filtered."""
        mock_get_json.return_value = [
            {"name": "repo1", "license": {"key": "mit"}},
            {"name": "repo2", "license": {"key": "apache-2.0"}},
        ]
        with patch(
            "client.GithubOrgClient._public_repos_url", new="mock_url"
        ):
            client = GithubOrgClient("test")
    ...
    def test_has_license(self, _, repo, license_key, expected):
        """Test has_license checks if repo has given license key."""
        self.assertEqual(
...
class TestIntegrationGithubOrgClient(unittest.TestCase):
    ...
    def test_public_repos_with_license(self):
        """Test public_repos filters correctly by license."""
        client = GithubOrgClient("google")
        self.assertEqual(
